...
/*
 * PSClientRequest.java
 *
 * Yannis Stavrakas (c) 2002
 */
...
class PSClientRequest 
{
	...
	public PSClientRequest(InetAddress dest, int port, String request, boolean methodPost, int timeout) 
	{
		...
		respCode = dispatchServer(dest, port, request, methodPost, response, timeout);
		//deal with possible errors while dispatching request
		errMsg = isError()? response.substring(0): "";  //if error 'response' contains err msg
		if (isError()) response.setLength(0);           //if error reset 'response'
		//parse response
		if ( ! isError()) parseResponse();  //if error 'ra' remains null
	}
  ...
  int dispatchServer(InetAddress dest, int port, String request, 
                     boolean methodPost, StringBuffer response, int timeout) 
	{
      ...
      fullRequest = "POST " + resource + " HTTP/1.0\nHost: " 
    ...
    try 
    {
      ...
      for (int ch = in.read(); ch > 0; ch = in.read())  //blocks execution
